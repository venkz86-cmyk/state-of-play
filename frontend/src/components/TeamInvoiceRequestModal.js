...
export const TeamInvoiceRequestModal = ({ open, onClose, token, companyName, planName }) => {
  const [legalName, setLegalName] = useState(companyName || '');
  const [hasGstin, setHasGstin] = useState(true); // teams usually have one
  ...
  const onSubmit = async (e) => {
    ...
    if (!legalName.trim() || !address.trim()) {
      setError('Please fill the legal name and address.');
      return;
    }
    if (hasGstin && !isInternational && !GSTIN_RE.test(gstin.trim().toUpperCase())) {
      setError('Enter a valid 15-character GSTIN, or untick the GSTIN box.');
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/api/invoice/generate-team`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          token,
          legal_name: legalName.trim(),
          gstin: hasGstin && !isInternational ? gstin.trim().toUpperCase() : null,
      ...
      a.download = m ? m[1] : 'team-tax-invoice.pdf';
  ...
  return (
    <div
      data-testid="team-invoice-modal"
      role="dialog"
      aria-modal="true"
      ...
      >
        <div className="flex items-baseline justify-between px-6 py-4 border-b border-[var(--rule)]">
          <p className="font-plex text-[11px] uppercase tracking-[0.08em] text-[var(--text-label)]">
            GST tax invoice · {planName}
          </p>
          <button
            type="button"
            onClick={handleClose}
            data-testid="team-invoice-modal-close"
        ...
        <form onSubmit={onSubmit} className="px-6 py-6 space-y-6">
          ...
          {/* Legal name (pre-filled from Apps Script) */}
          <div>
            ...
            <p className="font-plex text-[12px] text-[var(--text-label)] mt-2">
              Edit only if the GST-registered legal name differs from the trade name.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <input
              id="team-invoice-intl"
              type="checkbox"
              checked={isInternational}
              onChange={(e) => setIsInternational(e.target.checked)}
              data-testid="team-invoice-is-international"
              className="h-4 w-4 accent-[var(--accent-burgundy)]"
            />
            <label htmlFor="team-invoice-intl" className="font-plex text-[14px] text-[var(--text)]">
          ...
          {!isInternational && (
            <>
              <div className="flex items-center gap-3">
                <input
                  id="team-invoice-has-gstin"
                  type="checkbox"
                  checked={hasGstin}
                  onChange={(e) => setHasGstin(e.target.checked)}
                  data-testid="team-invoice-has-gstin"
                  className="h-4 w-4 accent-[var(--accent-burgundy)]"
                />
                <label htmlFor="team-invoice-has-gstin" className="font-plex text-[14px] text-[var(--text)]">
                  Issue against company GSTIN (input tax credit)
                </label>
              </div>
              ...
              {hasGstin && (
                <div>
                  ...
                  <input
                    ...
                    maxLength={15}
                    data-testid="team-invoice-gstin"
              ...
              <div>
                ...
                <select
                  ...
                  data-testid="team-invoice-state"
                  className="w-full bg-transparent border-0 border-b border-[var(--text)] font-plex text-base py-2 focus:outline-none focus:border-[var(--accent-burgundy)]"
                >
          ...
          <div>
            ...
            <textarea
              ...
              data-testid="team-invoice-address"
          ...
          <div>
            ...
            <input
              ...
              data-testid="team-invoice-issue-date"
              className="w-full bg-transparent border-0 border-b border-[var(--text)] font-plex text-base py-2 focus:outline-none focus:border-[var(--accent-burgundy)]"
            />
            <p className="font-plex text-[12px] text-[var(--text-label)] mt-2">
              Leave blank to use your subscription start date.
            </p>
          </div>

          {error && (
            <p className="font-plex text-[14px] text-[var(--accent-burgundy)]" data-testid="team-invoice-error">
              {error}
            </p>
          )}
          ...
          <div className="flex items-center gap-6 pt-2">
            <button
              type="submit"
              disabled={submitting}
              data-testid="team-invoice-submit"
...
export default TeamInvoiceRequestModal;
