...
export const TrialsPanel = ({ onAuthError }) => {
  ...
  const columns = [
    ...
    { key: 'opened_count', label: 'Read', sortable: true, align: 'right', render: (t) => t.opened_count ?? 0 },
    {
      key: 'reminders', label: 'Emails sent',
      render: (t) => [
        'welcome',
        t.progress_10day_sent && 'day-10',
        t.reminder_5day_sent && 'day-25',
        ...
        <div>
          ...
          {sweepResult && (
            <p className="font-plex text-[13px] text-[var(--text)] mt-2">
              {sweepResult.progress_sent} progress email{sweepResult.progress_sent === 1 ? '' : 's'} sent,{' '}
